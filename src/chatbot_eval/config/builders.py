...
"""Factory helpers for turning config files into runtime objects."""
...
def build_chat_client(model_config: ModelConfig):
    """Construct the appropriate chat client for ``model_config``."""
    ...
    if provider == 'ollama':
        return OllamaChatClient(
            ...
            request_kwargs=model_config.request_kwargs,
        )
    if provider == 'openai':
...
def build_judge_metric(project_root: str | Path, judge_config_path: str | Path) -> LLMJudgeMetric:
    ...
    return LLMJudgeMetric(
        ...
        llm_client=client,
        prompt_path=project_root / judge_config.prompt_path,
